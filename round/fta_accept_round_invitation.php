...
function accept_round_invitation(
    PDO $pdo,
    int $invusr_id,
    int $usr_id
): void {
    try {
        ...
        if ($invitation["invusr_status"] !== "invited") {
            $pdo->rollBack();
            sendErrorMessage(409, "Je hebt al op deze uitnodiging gereageerd.");
        }
        ...
        if ($affected_rows !== 1) {
            throw new RuntimeException(
                "De uitnodiging kon niet worden bijgewerkt."
            );
        }

        $pdo->commit();
        sendSuccessMessage("Je hebt de uitnodiging geaccepteerd.", [
            "data" => [
                "invusr_id" => $invusr_id,
                "invite_round_id" =>
        ...
        sendErrorMessage(500, "De uitnodiging kon niet worden geaccepteerd.", ["error_debug" => [
